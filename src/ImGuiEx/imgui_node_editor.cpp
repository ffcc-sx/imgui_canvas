...
# if IMGUI_VERSION_NUM >= 17909
    if (m_IsHoveredWithoutOverlapp)
        // Update for 1.89 compatible
        // ImGui::SetItemUsingMouseWheel();
        ImGui::SetItemKeyOwner(ImGuiKey_MouseWheelY);  // Update since 1.89.
# endif
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control &control) {
    ...
    auto &io = ImGui::GetIO();
    // SX: Append static cast for ImGuiKey compatible.
    if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(static_cast<ImGuiKey>(GetKeyIndexForF())) &&
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control &control) {
    ...
    // SX: Append static cast for compatible.
